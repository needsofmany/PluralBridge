...
SELECT '003 key and constraint inventory validation' AS validation_step;
...
DECLARE @actual_pk TABLE (TableName sysname NOT NULL, ConstraintName sysname NOT NULL, ColumnList nvarchar(max) NOT NULL);
INSERT INTO @actual_pk (TableName, ConstraintName, ColumnList)
SELECT
    t.name AS TableName,
...
DECLARE @actual_fk TABLE
(
    ConstraintName sysname NOT NULL,
    ChildTable sysname NOT NULL,
    ChildColumns nvarchar(max) NOT NULL,
    ParentTable sysname NOT NULL,
    ParentColumns nvarchar(max) NOT NULL,
    is_disabled bit NOT NULL,
    is_not_trusted bit NOT NULL
);

INSERT INTO @actual_fk (ConstraintName, ChildTable, ChildColumns, ParentTable, ParentColumns, is_disabled, is_not_trusted)
SELECT
...
DECLARE @actual_uq TABLE (TableName sysname NOT NULL, ConstraintName sysname NOT NULL, ColumnList nvarchar(max) NOT NULL);
INSERT INTO @actual_uq (TableName, ConstraintName, ColumnList)
SELECT
    t.name AS TableName,
    kc.name AS ConstraintName,
    STUFF((
...
DECLARE @actual_default TABLE (TableName sysname NOT NULL, ConstraintName sysname NOT NULL, ColumnName sysname NOT NULL);
INSERT INTO @actual_default (TableName, ConstraintName, ColumnName)
SELECT
    t.name AS TableName,
