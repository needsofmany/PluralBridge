-- 004_anonymize_member_descriptions_lorem_fixed.sql
-- Purpose: Replace dbo.pb_members.Description with same-character-count lorem ipsum text.
...
-- Fix: Uses DATALENGTH/2 instead of LEN so trailing spaces in the replacement text do not create a false length failure.
...
(
    ...
    WHERE
        ...
        AND (DATALENGTH(CONVERT(nvarchar(max), m.[Description])) / 2) <> r.OriginalLength
)
BEGIN
...
SELECT
    'PASS: member descriptions replaced with same-character-count lorem ipsum text.' AS validation_result,
    COUNT(*) AS pb_members,
